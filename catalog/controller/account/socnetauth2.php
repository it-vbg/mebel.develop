...
class ControllerAccountsocnetauth2 extends Controller {
	...
	public function frame()
	{
		...
		$socnetauth_data = unserialize( $this->getParam('socnetauth2_confirmdata') );
	...
	public function showcode($ar=array())
	{
		...
		if( empty($this->session->data['customer_id']) && 
			!empty($this->request->cookie['customer_id']) &&
			!empty($this->session->data['socnetauth2_lastlink']) 
		)
		{
			unset($this->session->data['socnetauth2_lastlink']);
			$this->session->data['customer_id'] = $this->request->cookie['customer_id'];
				
			$this->request->cookie['customer_id'] = '';
		...
		if( 
			...
			$this->getParam('socnetauth2_confirmdata') && 
			$this->getParam('socnetauth2_confirmdata_show') 
		)
		{
			
			$data = $this->getParam('socnetauth2_confirmdata');
			...
			$socnetauth2_confirm_block = str_replace("#divframe_height#", (340-(32*(5-(count($data))))), $socnetauth2_confirm_block );
	
			$socnetauth2_confirm_block = str_replace("#frame_height#", (360-(32*(5-(count($data))))), $socnetauth2_confirm_block);
	...
	public function showcode2($ar=array())
	{
		...
		if( 
			...
			$this->getParam('socnetauth2_confirmdata_show')  )
		{
			$data = unserialize( $this->getParam('socnetauth2_confirmdata') );
	...
	private function getParam($key)
	{
		...
		{
			return html_entity_decode($this->request->cookie[ $key ]);
		}
	}
