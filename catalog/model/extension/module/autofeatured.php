	...
	public function getProductAutoFeatured($product_id,$limit,$attributesset,$categoriesset_source,$categoriesset,$sortby,$in_stock,$one_cat) {
      ...
      if (isset($in_stock) && $in_stock == 1) {
        $sql .= " AND pr.quantity > 0";
			...
			$sql .= " GROUP BY cpr.product_id,pr.price,cpr.price"; 
      $sql .= " UNION ALL SELECT  RAND() as rnd,cpr.product_id,cpr.price,pr.price as cprice, 1 as quan FROM " . DB_PREFIX . "product pr LEFT JOIN " . DB_PREFIX . "product cpr ON (pr.product_id <> cpr.product_id and cpr.status = 1) 
