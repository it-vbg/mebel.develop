    ...
    <div class="<?php echo $page == 'checkout' ? 'col-sm-8' : 'col-sm-10' ?>">
      ...
      <?php } elseif ($type == 'radio') { ?>
        ...
        <?php foreach ($values as $info) { ?>
          <div class="radio">
            <label>
                <input type="radio" name="<?php echo $name ?>" id="<?php echo $id ?>" value="<?php echo $info['id'] ?>" <?php echo $value == $info['id'] ? 'checked="checked"' : '' ?> <?php echo $reload ? 'data-onchange="reloadAll"' : 'data-reload-payment-form="true"'?>><?php echo $info['text'] ?>
                <span></span>
            </label>
          </div>
        <?php } ?>
