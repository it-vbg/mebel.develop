        ...
        <?php if (!empty($shipping_methods)) { ?>
            ...
            <?php } else { ?>
                ...
                <?php foreach ($shipping_methods as $shipping_method) { ?>
                    ...
                    <?php if (empty($shipping_method['error'])) { ?>
                        <?php foreach ($shipping_method['quote'] as $quote) { ?>
                            <?php if ($quote['title'] == 'Самовывоз') {?>
                                <div class="radio svernuto" id="samovyvoz">
                                    <label for="<?php echo $quote['code']; ?>">
                                        <input type="radio" class="ventaform" data-onchange="reloadAll" name="shipping_method" <?php echo !empty($quote['dummy']) ? 'disabled="disabled"' : '' ?> <?php echo !empty($quote['dummy']) ? 'data-dummy="true"' : '' ?> value="<?php echo $quote['code']; ?>" id="<?php echo $quote['code']; ?>" <?php if ($quote['code'] == $code) { ?>checked="checked"<?php } ?> />
                                        <span></span> <?php echo !empty($quote['title']) ? $quote['title'] : ''; ?><?php echo !empty($quote['text']) && !$hide_cost ? ' - ' . $quote['text'] : ''; ?> <b>(Санкт-Петербург, ул.Бухарестская 24,к2)</b>

                                    </label>
                                    <button class="button btn-simple button_description_1 btn">открыть карту</button>
                                    <button class="button btn-simple button_description_2 btn svernuto">закрыть карту</button>
                                    <div id="samovyvoz-map">
                                    <script type="text/javascript" charset="utf-8" async src="https://api-maps.yandex.ru/services/constructor/1.0/js/?um=constructor%3A439975a23caf1fd011cc0acf4c42ce28ad16ac3f13d170f73a3a1d8533cb5082&amp;width=100%25&amp;height=300&amp;lang=ru_RU&amp;scroll=true"></script>
                                    </div>
                                </div>
...
<script type="text/javascript">
    $(function () {
        $('.button_description_1').click(function () {
            $('#samovyvoz, .button_description_2, .button_description_1').toggleClass('svernuto');
        });
        $('.button_description_2').click(function () {
            $('#samovyvoz, .button_description_1, .button_description_2').toggleClass('svernuto');
        });
    });
</script>