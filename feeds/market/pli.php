...
{
	$row_category['name_ru-RU'] = mb_convert_encoding ($row_category['name_ru-RU'] ,"UTF-8" , "Windows-1251" );
...
$array_tovar=mysql_query("select * from s2mp1_jshopping_products where product_price >= '0'");
...
$pd="";
$al="";
$result340=mysql_query("select * from s2mp1_jshopping_products_to_categories where product_id='".$row_tovar['product_id']."' GROUP BY product_id");
...
//}
		...
		$row[$i]['product_name']=str_replace ('\'','&apos;', $row[$i]['product_name']);
*/
		if ($row_tovar['product_old_price']>$row_tovar['product_price']){
		echo"<oldprice>".$row_tovar['product_old_price']."</oldprice>\n";
		}

        $row_tovar['name_ru-RU'] = mb_convert_encoding ($row_tovar['name_ru-RU'] ,"UTF-8" , "Windows-1251" );
		echo"<name>".$row_tovar['name_ru-RU']."</name>\n";
