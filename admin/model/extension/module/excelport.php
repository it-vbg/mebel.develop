...
class ModelExtensionModuleExcelport extends Model {
 ...
 //        }
	...
	public function getTabs() {
		...
		$name_map = array(
			'tab_export.php' => array(
				'name' => 'Export',
				'id' => 'export'
			),
			'tab_import.php' => array(
				'name' => 'Import',
				'id' => 'import'
			),
			'tab_settings.php' => array(
				'name' => 'Settings',
				'id' => 'settings'
			),
	...
	public function importXLS($type, $language, $file = '', $settings, $addAsNew = false) {
		...
		switch ($type) {
			...
			case 'Options' : {
				...
				$this->model_extension_module_excelport_option->importXLSOptions($language, $languages, $file);
			} break;
			case 'Attributes' : {
				$this->load->model('extension/module/excelport_attribute');
				$this->model_extension_module_excelport_attribute->importXLSAttributes($language, $languages, $file);
			} break;
			case 'Customers' : {
				$this->load->model('extension/module/excelport_customer');
				$this->model_extension_module_excelport_customer->importXLSCustomers($language, $languages, $file, $settings['ImportLimit']);
			} break;
			case 'CustomerGroups' : {
				$this->load->model('extension/module/excelport_customer_group');
				$this->model_extension_module_excelport_customer_group->importXLSCustomerGroups($language, $languages, $file, $settings['ImportLimit']);
			} break;
			case 'Orders' : {
            ...
            case 'Manufacturers' : {
                ...
                $this->model_extension_module_excelport_manufacturer->importXLSManufacturers($file, $settings['ImportLimit']);
            } break;
            case 'Coupons' : {
                $this->load->model('extension/module/excelport_coupon');
                $this->model_extension_module_excelport_coupon->importXLSCoupons($file, $settings['ImportLimit']);
            } break;
            case 'Vouchers' : {
                $this->load->model('extension/module/excelport_voucher');
                $this->model_extension_module_excelport_voucher->importXLSVouchers($file, $settings['ImportLimit']);
            } break;
		}
	}
	public function exportXLS($type, $language, $store, $destinationFolder = '', $settings, $exportMode = 0, $filter = false, $filters = array()) {
		...
		switch ($type) {
            ...
            case 'Manufacturers' : {
                $this->load->model('extension/module/excelport_manufacturer');
                $this->model_extension_module_excelport_manufacturer->exportXLSManufacturers($store, $destinationFolder, $settings['ExportLimit'], !empty($filters['Manufacturers']) && $filter ? $filters['Manufacturers'] : array());   
            } break;
            case 'Coupons' : {
                $this->load->model('extension/module/excelport_coupon');
