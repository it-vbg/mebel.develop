	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) 
		{
			...
			if( !empty($this->request->post['stay']) )
			{
				...
				$this->response->redirect($this->url->link('extension/module/socnetauth2', 'token=' . $this->session->data['token'].'&tab='.$tab, 'SSL'));
			}
			else
			{
		...
		{ 
			$this->data['socnetauth2_protokol'] = $this->config->get('socnetauth2_protokol');
		}
		/* end metka-606 */
		/* start metka: r1 */
		...
		$this->data['vkontakte_retargeting'] = $this->url->link('extension/module/socnetauth2/vkontakte_retargeting', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['facebook_retargeting'] = $this->url->link('extension/module/socnetauth2/facebook_retargeting', 'token=' . $this->session->data['token'], 'SSL');
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/socnetauth2', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('extension/module/socnetauth2', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('extension/module/socnetauth2.tpl', $this->data));
	}
