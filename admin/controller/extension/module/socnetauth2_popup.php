...
class ControllerExtensionModuleSocnetauth2Popup extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('socnetauth2_popup', $this->request->post);
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/socnetauth2_popup', 'token=' . $this->session->data['token'], 'SSL')
		);

		$this->data['action'] = $this->url->link('extension/module/socnetauth2_popup', 'token=' . $this->session->data['token'], 'SSL');
		...
		} elseif ($this->config->has('socnetauth2_popup_label')) { 
			$this->data['socnetauth2_popup_label'] = $this->custom_unserialize($this->config->get('socnetauth2_popup_label'));
		} else {
		...
		$this->response->setOutput($this->load->view('extension/module/socnetauth2_popup.tpl', $this->data));
	}
