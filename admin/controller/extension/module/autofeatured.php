...
class ControllerExtensionModuleAutoFeatured extends Controller {
    ...
    $data['adaptation'] = $this->url->link('extension/module/autofeatured/adaptation', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/autofeatured', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/autofeatured', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/autofeatured', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/module/autofeatured', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('extension/module/autofeatured.tpl', $data));
	}
  ...
  <code>autofeatured</code>
  <version>1.0</version>
  <author>2304 Studio</author>
  <link>http://2304.ru</link>
   <file path="catalog/controller/extension/module/autofeatured.php">';
      foreach ($xml as $xml) {
				if (empty($xml)){
					continue;
				}
            ...
            $path = '';
                  ...
                  if (substr($file, -13) == '/featured.php') {
                    foreach ($operations as $operation) {
                        $xmlstr .= '
      ' . str_replace("/featured","/autofeatured",$operation->ownerDocument->saveXML($operation));
                    }
                  }
                }
              }
...
$xmlstr .= '	</file>
      ...
      $this->response->addHeader('Content-Disposition: attachment; filename=autofeatured.ocmod.xml');
